...
float sideLong = 0;        // ระยะด้าน AB
float sideShort = 0;       // ระยะด้าน BC
float sideShortStart = 0;  // ค่าเริ่มต้นด้าน BC
float shrinkStep = 0.4;   // ลดกรอบครั้งละเท่าไร (เหมือน BT)
...
void computeCorners() {
    ...
    sideLong = (unsigned long)(distAB * timePerMeter);
    sideShort = (unsigned long)(distBC * timePerMeter);
...
void prepareLegs() {
    ...
    Serial.printf("tAB=%lu   tBC=%lu   tCD=%lu   tDA=%lu\n",
        runDuration[0], runDuration[1], runDuration[2], runDuration[3]);
}
...
float getHeading() {
    // คุณใช้ heading จากตัวแปร gyro_heading อยู่แล้ว
    return gyro_heading;  
}

// กำหนดเป้าหมายตาม state
float getTurnTarget(int legState) {
    switch (legState) {

        case 0:  // หมุนจาก AB → BC
            return 270.0;

        case 2:  // หมุนจาก BC → CD
            return 180.0;

        case 4:  // หมุนจาก CD → DA
            return 90.0;

        case 6:  // หมุนจาก DA → AB
            return 0.0;
    }
    return 0.0;
}

void turnToDeg(float targetDeg) { 
    
    float target = targetDeg;  // ใช้ค่าที่ส่งมาแทน -90
    Serial.printf("TURN TO: target=%.2f\n", target);
    ...
    while (true) {
        ...
        float h = gyro_heading;

        float diff = norm360(h - target);

        Serial.printf("H=%.2f target=%.2f diff=%.2f\n", h, target, diff);
...
/////////////////////////////คุมมอเตอร์/////////////////////////
    ...
    case 1: {
        ...
        unsigned long used  = millis() - btTimer;
        float remain = (sideLong * 1000 - used) / 1000.0;
        if (remain < 0) remain = 0;

        Serial.printf("⏳ Turning in %.2f sec\n", remain);

        if (used >= (unsigned long)(sideLong * 1000)) {
            if (sideShort < 0) sideShort = 0;
            sideShort -= shrinkStep;
            turnToDeg(270.0);
    ...
    case 3: {
        ...
        unsigned long used = millis() - btTimer;
        float remain = (sideShort * 1000 - used) / 1000.0;
        if (remain < 0) remain = 0;

        Serial.printf("⏳ Turning in %.2f sec\n", remain);

        if (used >= (unsigned long)(sideShort * 1000)) {
            turnToDeg(180.0);
            btState = 4;
        }
        break;
